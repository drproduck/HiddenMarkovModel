...
public class Decode {
    ...
    public static void main(String[] args) throws Exception{
        double[][] transition = Helper.readBigram("data/bigram");
        double[] pi = Helper.readPi("data/monogram");
        int[] observations = Helper.read408("homophonic_408");
        ...
        Scanner scanner;
        File dir = new File("/home/drproduck/Documents/em.HMM/bfgs_proposals/proposals");
    ...
    public static int[] viterbi(double[] pi, double[][] transition, double[][] emission, int[] observations, int length, int n_tags) {
        ...
        for (int i = 0; i < n_tags; i++) {
            head[i] = java.lang.Math.log(pi[i]) + 3 * java.lang.Math.log(emission[i][observations[0]]);
        }
        ...
        for (int t = 1; t < length; t++) {
            for (int i = 0; i < n_tags; i++) {
                ...
                for (int j = 0; j < n_tags; j++) {
                    x = head[j] + java.lang.Math.log(transition[j][i]);
                    if (x > max) {
                        max = x;
                        argmax = j;
                    }
                ...
                dp[t][i] = argmax;
                head[i] = max + 3 * java.lang.Math.log(emission[i][observations[t]]);
            }
        }
